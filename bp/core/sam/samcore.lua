                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('WS') and helpers['actions'].canAct() and target and (target.distance):sqrt() < 6 then
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif (helpers['buffs'].buffActive(aftermath) or not weaponskill) and current.tp >= self.getSetting('TP THRESHOLD') and helpers['actions'].isReady('WS', self.getSetting('WSNAME')) then
                                ...
                                -- HAGAKURE.
                                elseif helpers['buffs'].buffActive(483) and helpers['actions'].isReady('WS', bp.JA["Hagakure"]) and not helpers['queue'].inQueue(bp.JA["Hagakure"]) then
                                    helpers['queue'].addToFront(bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp.JA["Hagakure"], player)

                                -- SENGIKORI.
                                elseif helpers['buffs'].buffActive(483) and helpers['actions'].isReady('WS', bp.JA["Sengikori"]) and not helpers['queue'].inQueue(bp.JA["Sengikori"]) then
                                    helpers['queue'].addToFront(bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp.JA["Sengikori"], player)
                    ...
                    if self.getSetting('JA') and helpers['actions'].canAct() then
                        ...
                        if player.main_job == 'SAM' and helpers['actions'].canAct() then
                            ...
                            -- MEDITATE.
                            if helpers['actions'].isReady('JA', "Meditate") and player['vitals'].tp < 2500 then
                                helpers['queue'].add(bp.JA["Meditate"], player)
                            end
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == 'SAM' and helpers['actions'].canAct() then
                            ...
                            -- THIRD EYE.
                            if not helpers['buffs'].buffActive(67) and helpers['actions'].isReady('JA', "Third Eye") then
