...
function core.get()
    ...
    self["SKILLS"]              = self.settings["SKILLS"] or {{"Enhancing"}, "Enhancing"}
                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('SKILLUP') then
                        ...
                        if helpers['inventory'].findItemByName("B.E.W. Pitaru") and not helpers['queue'].inQueue(IT["B.E.W. Pitaru"], player) and not helpers['buffs'].buffActive(251) then
                            ...
                            if bp.skillup[self.getSetting('SKILLUP')].list then

                                for i,v in pairs(bp.skillup[self.getSetting('SKILLUP')].list) do
        
                                    if helpers['actions'].isReady(bp, "MA", v) then
                                        helpers['queue'].add(bp, bp.MA[v], bp.skillup[self.getSetting('SKILLUP')].target)
                                    end
                    ...
                    if self.getSetting('WS') and helpers['actions'].canAct() and target and (target.distance):sqrt() < 6 then
                        local current = {tp=player["vitals"].tp, hpp=player["vitals"].hpp, mpp=player["vitals"].mpp, main=helpers['equipment'].main.en, ranged=helpers['equipment'].main.en}
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif not helpers['buffs'].buffActive(bp, aftermath) and current.tp > self.getSetting('AM THRESHOLD') and weaponskill and helpers['actions'].isReady(bp, 'WS', weaponskill) then
                                helpers["queue"].addToFront(bp, bp.WS[weaponskill], target)
                                
                            elseif (helpers['buffs'].buffActive(bp, aftermath) or not weaponskill) and current.tp > self.getSetting('TP THRESHOLD') and helpers['actions'].isReady(bp, 'WS', self.getSetting('WSNAME')) then
                                helpers["queue"].addToFront(bp, bp.WS[self.getSetting('WSNAME')], target)
                                
                            end
                            
                        elseif not self.getSetting('AM') and target and (target.distance):sqrt() < 6 then
                                
                            if current.tp > self.getSetting('TP THRESHOLD') and helpers["actions"].isReady(bp, "WS", self.getSetting('WSNAME')) then
                                helpers["queue"].addToFront(bp, bp.WS[self.getSetting('WSNAME')], target)                            
                            end
                        
                        elseif not self.getSetting('AM') and target and (target.distance):sqrt() > 6 then
                            
                            if current.tp > self.getSetting('TP THRESHOLD') and helpers["actions"].isReady(bp, "WS", self.getSetting('RANGED WS')) then
                                helpers["queue"].addToFront(bp, bp.WS[settings["RANGED WS"]], target)                            
                            end
                ...
                elseif player.status == 0 then
                    ...
                    if self.getSetting('WS') and helpers['actions'].canAct() and target and (target.distance):sqrt() < 6 then
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif not helpers['buffs'].buffActive(bp, aftermath) and current.tp > (self.getSetting('AM LEVEL')*1000) and weaponskill and helpers['actions'].isReady(bp, 'WS', weaponskill) then
                                helpers["queue"].addToFront(bp, bp.WS[weaponskill], target)
                    ...
                    if self.getSetting('JA') and helpers['actions'].canAct() then
                        ...
                        if player.main_job == 'RUN' then                        
                            ...
                            if helpers['runes'].active:length() > 0 then
                                ...
                                if (runes[1] and runes[1].en ~= "Tenebrae") or (runes[2] and runes[2].en ~= "Tenebrae") or (runes[3] and runes[3].en ~= "Tenebrae") then
                                    ...
                                    -- VIVACIOUS PULSE.
                                    if helpers["actions"].isReady(bp, "JA", "Vivacious Pulse") and player['vitals'].hpp < self.getSetting('VPULSE HPP') then
                                        helpers["queue"].add(bp, bp.JA["Vivacious Pulse"], player)
                                    end
                                    
                                elseif (runes[1] and runes[1].en == "Tenebrae") or (runes[2] and runes[2].en == "Tenebrae") or (runes[3] and runes[3].en == "Tenebrae") then
                                    
                                    -- VIVACIOUS PULSE.
                                    if helpers["actions"].isReady(bp, "JA", "Vivacious Pulse") and player['vitals'].mpp < self.getSetting('VPULSE MPP') then
                                        helpers["queue"].add(bp, bp.JA["Vivacious Pulse"], player)
                                    end
                                ...
                                -- BATTUTA.
                                if target and helpers["actions"].isReady(bp, "JA", "Battuta") then
                                    helpers["queue"].add(bp, bp.JA["Battuta"], player)
                                end
                    ...
                    if self.getSetting('HATE') and target then
                        ...
                        if player.main_job == "RUN" then
                            ...
                            -- FLASH.
                            if helpers['actions'].canCast() and helpers["actions"].isReady(bp, "MA", "Flash") then
                                helpers["queue"].addToFront(bp, bp.MA["Foil"], target)
                            
                            -- FOIL.
                            elseif helpers['actions'].canCast() and helpers["actions"].isReady(bp, "JA", "Foil") then
                                helpers["queue"].addToFront(bp, bp.MA["Foil"], player)
                            ...
                            if helpers['actions'].canAct() and (os.clock()-timers.hate) > self.getSetting('HATE DELAY') then
                                ...
                                -- VALIANCE.
                                elseif target and helpers["actions"].isReady(bp, "JA", "Valiance") and helpers['runes'].active:length() > 0 then
                                    helpers["queue"].addToFront(bp, bp.JA["Valiance"], player)
                                    timers.hate = os.clock()
                                    
                                -- LIEMENT.
                                elseif target and helpers["actions"].isReady(bp, "JA", "Liement") and helpers['runes'].active:length() > 0 then
                                    helpers["queue"].addToFront(bp, bp.JA["Liement"], player)
                                    timers.hate = os.clock()
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == "RUN" then
                            ...
                            -- PHALANX.
                            if helpers['actions'].canCast() and not helpers['buffs'].buffActive(116) and helpers['actions'].isReady(bp, "MA", "Phalanx") then
                                
                                if helpers['actions'].canAct() and self.getSetting('EMBOLDEN') == "Phalanx" and not helpers['buffs'].buffActive(534) and helpers['actions'].isReady(bp, "JA", "Embolden") then
                                    helpers['queue'].add(bp, bp.JA["Embolden"], player)
                            ...
                            -- CRUSADE.
                            elseif helpers['actions'].canCast() and not helpers['buffs'].buffActive(289) and helpers['actions'].isReady(bp, "MA", "Crusade") then
                                helpers['queue'].add(bp, bp.MA["Crusade"], player)
                                
                            -- TEMPER.
                            elseif helpers['actions'].canCast() and not helpers['buffs'].buffActive(432) and helpers['actions'].isReady(bp, "MA", "Temper") then
                                
                                if helpers['actions'].canAct() and self.getSetting('EMBOLDEN') == "Temper" and not helpers['buffs'].buffActive(534) and helpers['actions'].isReady(bp, "JA", "Embolden") then
                                    helpers['queue'].add(bp, bp.JA["Embolde"], player)
                                    helpers['queue'].add(bp, bp.MA["Temper"], player)
                                else
                                    helpers['queue'].add(bp, bp.MA["Temper"], player)
                                end
                                
                            -- REGEN IV.
                            elseif helpers['actions'].canCast() and not helpers['buffs'].buffActive(42) and helpers['actions'].isReady(bp, "MA", "Regen IV") then
                                
                                if helpers['actions'].canAct() and self.getSetting('EMBOLDEN') == "Regen IV" and not helpers['buffs'].buffActive(534) and helpers['actions'].isReady(bp, "JA", "Embolden") then
                                    helpers['queue'].add(bp, bp.JA["Embolde"], player)
                            ...
                            -- REFRESH.
                            elseif helpers['actions'].canCast() and not helpers['buffs'].buffActive(43) and helpers['actions'].isReady(bp, "MA", "Refresh") then
                                helpers['queue'].add(bp, bp.MA["Refresh"], "me")
                                
                            -- SPIKES.
                            elseif helpers['actions'].canCast() and self.getSetting('SPIKES') ~= "None" and not helpers['buffs'].buffActive(34) and not helpers['buffs'].buffActive(35) and not helpers['buffs'].buffActive(38) then
                            
                                if helpers['actions'].isReady(bp, "MA", self.getSetting('SPIKES')) then
                        ...
                        if player.sub_job == "SCH" then
                            ...
                            if helpers['actions'].canAct() and helpers['actions'].isReady(bp, "JA", self.getSetting('ARTS')) and self.getSetting('ARTS') == "Light Arts" and (not helpers['buffs'].buffActive(358) and not helpers['buffs'].buffActive(401)) then
                                helpers['queue'].add(bp, bp.JA[settings["ARTS"]:current()], player)
                            ...
                            -- ADDENDUM.
                            elseif helpers['actions'].canAct() and (helpers['buffs'].buffActive(358) or helpers['buffs'].buffActive(359)) and helpers['actions'].isReady(bp, "JA", settings["WHITE"]:current()) and helpers["stratagems"].getStratagems().current > 0 then
