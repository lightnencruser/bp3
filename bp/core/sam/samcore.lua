                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('WS') and helpers['actions'].canAct() and target and (target.distance):sqrt() < 6 then
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif (helpers['buffs'].buffActive(aftermath) or not weaponskill) and current.tp >= self.getSetting('TP THRESHOLD') and helpers['actions'].isReady(bp, 'WS', self.getSetting('WSNAME')) then
                                ...
                                -- HAGAKURE.
                                elseif helpers['buffs'].buffActive(483) and helpers["actions"].isReady(bp, "WS", bp.JA["Hagakure"]) and not helpers['queue'].inQueue(bp, bp.JA["Hagakure"]) then
                                    helpers['queue'].addToFront(bp, bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp, bp.JA["Hagakure"], player)

                                -- SENGIKORI.
                                elseif helpers['buffs'].buffActive(483) and helpers["actions"].isReady(bp, "WS", bp.JA["Sengikori"]) and not helpers['queue'].inQueue(bp, bp.JA["Sengikori"]) then
                                    helpers['queue'].addToFront(bp, bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp, bp.JA["Sengikori"], player)
                    ...
                    if self.getSetting('JA') and helpers['actions'].canAct() then
                        ...
                        if player.main_job == 'SAM' and helpers['actions'].canAct() then
                            ...
                            -- MEDITATE.
                            if helpers["actions"].isReady(bp, "JA", "Meditate") and player['vitals'].tp < 2500 then
                                helpers["queue"].add(bp, bp.JA["Meditate"], player)
                            end
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == 'SAM' and helpers['actions'].canAct() then
                            ...
                            -- SEIGAN.
                            if self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(354) and helpers['actions'].isReady(bp, 'JA', "Seigan") then
                                helpers["queue"].add(bp, bp.JA["Seigan"], player)

                            --HASSO.
                            elseif not self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(353) and helpers['actions'].isReady(bp, 'JA', "Hasso") then
                                helpers["queue"].add(bp, bp.JA["Hasso"], player)

                            end

                            -- THIRD EYE.
                            if not helpers['buffs'].buffActive(67) and helpers['actions'].isReady(bp, 'JA', "Third Eye") then
