...
function core.get()
    ...
    self["WSNAME"]              = self.settings["WSNAME"] or "Moonlight"
    ...
    self["SEKKA"]               = self.settings["SEKKA"] or "Moonlight"
    ...
    self["COMPOSURE"]           = self.settings["COMPOSURE"] or {{true,false}, false}
                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('JA') and helpers['actions'].canAct() then
                        ...
                        if player.main_job == 'WHM' then
                            ...
                            -- AFFLATUS.
                            if self.getSetting('MISERY') and not helpers['buffs'].buffActive(418) and helpers['actions'].isReady('JA', "Afflatus Misery") then
                                helpers['queue'].add(bp.JA["Afflatus Misery"], player)

                            elseif not self.getSetting('MISERY') and not helpers['buffs'].buffActive(417) and helpers['actions'].isReady('JA', "Afflatus Solace") then
                                helpers['queue'].add(bp.JA["Afflatus Solace"], player)
                        ...
                        elseif player.sub_job == "SCH" then
                            ...
                            if self.getSetting('SUBLIMATION') then
                                ...
                                elseif not helpers['buffs'].buffActive(187) and helpers['buffs'].buffActive(188) and helpers['actions'].isReady('JA', "Sublimation") then
                                    helpers['queue'].addToFront(bp.JA["Sublimation"], player)
                            ...
                            if player.sub_job == 'SCH' then
                                ...
                                if (helpers['buffs'].buffActive(358) or helpers['buffs'].buffActive(359)) then
                                    ...
                                    if not helpers['buffs'].buffActive(113) then
                                        ...
                                        if self.getSetting('JOB POINTS') > 99 and helpers['actions'].isReady('MA', "Reraise IV") then
                                            helpers['queue'].add(bp.MA["Reraise IV"], player)

                                        elseif self.getSetting('JOB POINTS') < 100 and helpers['actions'].isReady('MA', "Reraise III") then
                                            helpers['queue'].add(bp.MA["Reraise III"], player)

                                        end

                                    -- HASTE.
                                    elseif not helpers['buffs'].buffActive(33) and helpers['actions'].isReady('MA', "Haste") then
                                        helpers['queue'].add(bp.MA["Haste"], player)

                                    -- PROTECT.
                                    elseif not helpers['buffs'].buffActive(40) and helpers['actions'].isReady('MA', "Protectra V") then
                                        helpers['queue'].add(bp.MA["Protectra V"], player)

                                    -- SHELL.
                                    elseif not helpers['buffs'].buffActive(41) and helpers['actions'].isReady('MA', "Shellra V") then
                                        helpers['queue'].add(bp.MA["Shellra V"], player)
