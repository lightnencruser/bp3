                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('WS') and helpers['actions'].canAct() and target and (target.distance):sqrt() < 6 then
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif (helpers['buffs'].buffActive(aftermath) or not weaponskill) and current.tp >= self.getSetting('TP THRESHOLD') and helpers['actions'].isReady('WS', self.getSetting('WSNAME')) then
                                ...
                                -- SEKKANOKI.
                                if helpers['buffs'].buffActive(408) and helpers['actions'].isReady('WS', bp.JA['Sekkanoki']) and not helpers['queue'].inQueue(bp.JA['Sekkanoki']) then
                                    helpers['queue'].addToFront(bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp.JA['Sekkanoki'], player)
                                
                                -- WARRIORS CHARGE.
                                elseif helpers['buffs'].buffActive(340) and helpers['actions'].isReady('WS', bp.JA["Warrior's Charge"]) and not helpers['queue'].inQueue(bp.JA["Warrior's Charge"]) then
                                    helpers['queue'].addToFront(bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp.JA["Warrior's Charge"], player)
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == 'WAR' and helpers['actions'].canAct() then
                            ...
                            -- RETALIATION.
                            if not helpers['buffs'].buffActive(405) and helpers['actions'].isReady('JA', "Retaliation") then
                                helpers['queue'].addToFront(bp.JA["Retaliation"], player)
                            end

                            -- DEFENDER.
                            if self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(57) and helpers['actions'].isReady('JA', "Defender") then
                                helpers['queue'].add(bp.JA["Defender"], player)

                            --BERSERK.
                            elseif target and not self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(56) and helpers['actions'].isReady('JA', "Berserk") then
                            ...
                            -- WARCRY.
                            elseif target and not helpers['buffs'].buffActive(68) and helpers['actions'].isReady('JA', "Warcry") and not helpers['queue'].inQueue(bp.JA['Blood Rage'], 'me') then
                                helpers['queue'].add(bp.JA["Warcry"], player)

                            -- RESTRAINT.
                            elseif target and not helpers['buffs'].buffActive(435) and helpers['actions'].isReady('JA', "Restraint") then
                                helpers['queue'].add(bp.JA["Restraint"], player)

                            -- BLOOD RAGE.
                            elseif target and not helpers['buffs'].buffActive(460) and helpers['actions'].isReady('JA', "Blood rage") and not helpers['queue'].inQueue(bp.JA['Warcry'], 'me') then
                                helpers['queue'].add(bp.JA["Blood Rage"], player)
