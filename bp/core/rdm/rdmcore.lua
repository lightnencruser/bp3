...
function core.get()
    ...
    self["MAGIC BURST"] = {
        ...
        ["Compression"] = {
            T{'Aspir','Drain','Frazzle','Frazzle II','Frazzle III','Blind II'},
            T{},
        },
        ["Liquefaction"] = {
            T{'Fire','Fire II','Fire III','Fire IV','Fire V','Addle','Addle II'},
            T{},
        },
        ["Scission"] = {
            T{'Stone','Stone II','Stone III','Stone IV','Stone V','Slow II'},
            T{},
        },
        ["Reverberation"] = {
        ...
        ["Detonation"] = {
            T{'Aero','Aero II','Aero III','Aero IV','Aero V','Silence II'},
            T{},
        },
        ["Induration"] = {
            T{'Blizzard','Blizzard II','Blizzard III','Blizzard IV','Blizzard V','Distract','Distract II','Distract III','Paralyze II'},
            T{},
        },
        ["Impaction"] = {
                ...
                if player.status == 1 then
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == 'RDM' and helpers['actions'].canCast() then
                            ...
                            -- COMPOSURE.
                            if not helpers['buffs'].buffActive(419) and helpers['actions'].isReady(bp, 'JA', 'Composure') then
                                helpers['queue'].addToFront(bp, bp.JA['Composure'], player)
                            end

                            -- REFRESH.
                            if not helpers['buffs'].buffActive(43) then

                                if self.getSetting('JOB POINTS') > 1199 and helpers['actions'].isReady(bp, 'JA', 'Refresh III') then
                                    helpers['queue'].add(bp, bp.MA['Refresh III'], player)

                                elseif self.getSetting('JOB POINTS') < 1200 and helpers['actions'].isReady(bp, 'JA', 'Refresh II') then
                                    helpers['queue'].add(bp, bp.MA['Refresh II'], player)
                            ...
                            -- HASTE II.
                            elseif not helpers['buffs'].buffActive(33) and helpers['actions'].isReady(bp, 'JA', 'haste II') then
                                helpers['queue'].add(bp, bp.MA['Haste II'], player)

                            -- PHALANX.
                            elseif not helpers['buffs'].buffActive(116) and helpers['actions'].isReady(bp, 'JA', 'Phalanx') then
                                helpers['queue'].add(bp, bp.MA['Phalanx'], player)

                            -- TEMPER II.
                            elseif not helpers['buffs'].buffActive(432) and helpers['actions'].isReady(bp, 'JA', 'Temper II') and self.getSetting('JOB POINTS') > 1199 then
                                helpers['queue'].add(bp, bp.MA['Temper II'], player)

                            -- TEMPER.
                            elseif not helpers['buffs'].buffActive(432) and helpers['actions'].isReady(bp, 'JA', 'Temper') and self.getSetting('JOB POINTS') < 1200 then
                                helpers['queue'].add(bp, bp.MA['Temper'], player)

                            -- ENSPELL.
                            elseif not helpers['buffs'].buffActive(432) and helpers['actions'].isReady(bp, 'JA', self.getSetting('ENSPELL')) then
                                helpers['queue'].add(bp, bp.MA[self.getSetting('ENSPELL')], player)

                            -- GAINS.
                            elseif not helpers['buffs'].buffActive(432) and helpers['actions'].isReady(bp, 'JA', self.getSetting('GAINS')) then
                                ...
                                if (not helpers['buffs'].buffActive(542) or not helpers['buffs'].buffActive(543) or not helpers['buffs'].buffActive(545) or not helpers['buffs'].buffActive(546) or not helpers['buffs'].buffActive(547) or not helpers['buffs'].buffActive(548)) then
                                    helpers['queue'].add(bp, bp.MA[self.getSetting('GAINS')], player)
                                end

                            -- SPIKES.
                            elseif self.getSetting('SPIKES') ~= 'None' and helpers['actions'].isReady(bp, 'JA', self.getSetting('SPIKES')) then
