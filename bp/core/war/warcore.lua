...
function core.get()
    ...
    self["WSNAME"]              = self.settings["WSNAME"] or "Resolution"
    ...
    self["SEKKA"]               = self.settings["SEKKA"] or "Resolution"
                ...
                if player.status == 1 then
                        ...
                        if self.getSetting('AM') then
                            ...
                            elseif (helpers['buffs'].buffActive(aftermath) or not weaponskill) and current.tp >= self.getSetting('TP THRESHOLD') and helpers['actions'].isReady(bp, 'WS', self.getSetting('WSNAME')) then
                                ...
                                if helpers['buffs'].buffActive(408) and helpers['actions'].isReady(bp, 'WS', bp.JA['Sekkanoki']) and not helpers['queue'].inQueue(bp, bp.JA['Sekkanoki']) then
                                    ...
                                    helpers['queue'].addToFront(bp, bp.JA['Sekkanoki'], player)
                                
                                -- WARRIORS CHARGE.
                                elseif helpers['buffs'].buffActive(340) and helpers['actions'].isReady(bp, 'WS', bp.JA["Warrior's Charge"]) and not helpers['queue'].inQueue(bp, bp.JA["Warrior's Charge"]) then
                                    helpers['queue'].addToFront(bp, bp.WS[self.getSetting('WSNAME')], target)
                                    helpers['queue'].addToFront(bp, bp.JA["Warrior's Charge"], player)
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        if player.main_job == 'WAR' and helpers['actions'].canAct() then
                            ...
                            -- RETALIATION.
                            if not helpers['buffs'].buffActive(405) and helpers['actions'].isReady(bp, 'JA', "Retaliation") then
                                helpers['queue'].addToFront(bp, bp.JA["Retaliation"], player)
                            end

                            -- DEFENDER.
                            if self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(57) and helpers['actions'].isReady(bp, 'JA', "Defender") then
                                helpers['queue'].add(bp, bp.JA["Defender"], player)

                            --BERSERK.
                            elseif target and not self.getSetting('TANK MODE') and not helpers['buffs'].buffActive(56) and helpers['actions'].isReady(bp, 'JA', "Berserk") then
                            ...
                            -- WARCRY.
                            elseif target and not helpers['buffs'].buffActive(68) and helpers['actions'].isReady(bp, 'JA', "Warcry") and not helpers['queue'].inQueue(bp, bp.JA['Blood Rage'], 'me') then
                                helpers['queue'].add(bp, bp.JA["Warcry"], player)

                            -- RESTRAINT.
                            elseif target and not helpers['buffs'].buffActive(435) and helpers['actions'].isReady(bp, 'JA', "Restraint") then
                                helpers['queue'].add(bp, bp.JA["Restraint"], player)

                            -- BLOOD RAGE.
                            elseif target and not helpers['buffs'].buffActive(460) and helpers['actions'].isReady(bp, 'JA', "Blood rage") and not helpers['queue'].inQueue(bp, bp.JA['Warcry'], 'me') then
                                helpers['queue'].add(bp, bp.JA["Blood Rage"], player)
                ...
                elseif player.status == 0 then
                    ...
                    if self.getSetting('BUFFS') then
                        ...
                        elseif player.sub_job == "NIN" then
                            ...
                            -- UTSUSEMI
                            if helpers['actions'].canCast() and helpers['inventory'].findItemByName("Shihei", 0) and self.getSetting('SHADOWS') then
